...
describe("database schema foundation", () => {
	...
	it("maps Better Auth core fields to sando schema tables", () => {
		...
		expect(userColumns.isAnonymous.getSQLType()).toBe("boolean")
		expect(userColumns.isAnonymous.notNull).toBe(true)
		expect(userColumns.isAnonymous.hasDefault).toBe(true)
